...
export async function PATCH(
  ...
  try {
    ...
    const unPublishedTestimonial = await db.testimonials.update({
      where: {
        id: testimonialId,
      },
      data: {
    ...
    return NextResponse.json(unPublishedTestimonial, { status: 200 });
  } catch (error) {
    console.log("[TESTIMONIAL_ID_UNPUBLISH]", error);
